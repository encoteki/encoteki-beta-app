  ...
  if (isFinalizing) {
    return (
      <ButtonShell variant="primary" disabled>
        <Loading label={isSigningIn ? 'Signing you in…' : 'Almost done…'} />
      </ButtonShell>
    )
  }
  ...
  if (isXellarPermissionPending) {
    ...
    return (
      <div className="flex w-full flex-col items-center gap-1.5 tablet:w-auto">
        <ButtonShell variant="primary" onClick={handleClick} disabled={!hasUrl}>
...
function ButtonShell({
  ...
  const styles = {
    ...
    muted: 'flex items-center justify-center bg-neutral-60/20 text-neutral-40',
  }[variant]
  return (
    <button
      onClick={onClick}
